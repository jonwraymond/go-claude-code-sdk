...
func TestNewAPIKeyAuthenticator(t *testing.T) {
	tests := []struct {
		...
		{
			name:    "valid api key",
			apiKey:  "sk-ant-api03-valid-test-key-123456789",
			wantErr: false,
		},
		{
			name:    "empty api key",
...
func TestAPIKeyAuthenticator_Authenticate(t *testing.T) {
	auth, err := NewAPIKeyAuthenticator("sk-ant-api03-valid-test-key-123456789")
	if err != nil {
	...
	if apiKey, ok := authInfo.Headers["X-API-Key"]; !ok || apiKey != "sk-ant-api03-valid-test-key-123456789" {
		t.Errorf("AuthInfo.Headers[X-API-Key] = %v, want %v", apiKey, "sk-ant-api03-valid-test-key-123456789")
	}
}
...
func TestAuthOption_WithRefreshBefore(t *testing.T) {
	...
	apiAuth, _ := NewAPIKeyAuthenticator("sk-ant-api03-valid-test-key-123456789")
	err = opt(apiAuth)
	if err == nil {
